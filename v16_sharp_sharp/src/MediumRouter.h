...
private:
    ...
    void sharp_routing(double remainingTime) {
        ...
        if (debug) {
            std::cout << "Starting ENHANCED sharp routing with " 
                      << remainingTime << " seconds remaining\n";
        }
        ...
        if (debug) {
            std::cout << "Found " << failedNets.size() 
                      << " failed nets for enhanced sharp routing\n";
        }
        
        // Even more generous time budget per net since we have very few nets
        const double maxTimePerNet = std::min(remainingTime / (failedNets.size() * 1.5), 300.0); // Up to 5 minutes per net
        
        // Ultra-aggressive parameters
        const int ultraMaxExpand = MAX_EXPAND * 20; // 20x normal expansion limit
        const int maxAttempts = 30; // Try many more variations
        
        // Track summary statistics
        int totalSuccessfulNets = 0;
        
        // Try the nets in different orders for multiple passes
        for (int pass = 0; pass < 3 && !failedNets.empty(); ++pass) {
            if (debug) {
                std::cout << "Starting sharp routing pass " << (pass + 1) 
                          << " with " << failedNets.size() << " remaining nets\n";
            }
            
            // Try different ordering strategies for each pass
            if (pass == 0) {
                // First pass: sort by sink count (fewer sinks first)
                std::sort(failedNets.begin(), failedNets.end(), 
                         [&](size_t a, size_t b) {
                             return nets[a].sinks.size() < nets[b].sinks.size();
                         });
            } else if (pass == 1) {
                // Second pass: sort by estimated difficulty (easier first)
                std::sort(failedNets.begin(), failedNets.end(), 
                         [&](size_t a, size_t b) {
                             double distA = 0, distB = 0;
                             for (int sink : nets[a].sinks) {
                             ...
                             return (distA / nets[a].sinks.size()) < 
                                    (distB / nets[b].sinks.size());
                         });
            } else {
                // Third pass: reverse order (hardest first)
                std::reverse(failedNets.begin(), failedNets.end());
            }
            
            // Keep track of which nets were fixed in this pass
            std::vector<size_t> fixedInThisPass;
            ...
            for (size_t idx : failedNets) {
                ...
                if (debug) {
                    std::cout << "Pass " << (pass + 1) << ": Attempting ultra-sharp routing for net " 
                              << net.id << " (" << net.name << ") with " 
                              << net.sinks.size() << " sinks\n";
                }
                ...
                // Most aggressive A* search with virtually no congestion consideration
                auto ultraSharpAStar = [&](const std::unordered_set<int>& sources, int goal,
                                           double randomFactor, double congestionWeight) -> std::vector<int> {
                    std::priority_queue<PQ> pq;
                    std::unordered_map<int, double> g;
                    std::unordered_map<int, int> par;
                    ...
                    int expand = 0;
                    while (!pq.empty() && expand < ultraMaxExpand) {
                        auto cur = pq.top(); pq.pop();
                        if (cur.n == goal) break;
                        if (cur.g != g[cur.n]) continue;
                        ++expand;
                        ...
                        for (int nb : adj[cur.n]) {
                            // Minimal congestion consideration with randomization
                            double r = (std::rand() % 100) / 100.0 * randomFactor;
                            double penalty = congestionWeight * occ[nb] + r;
                            double ng = cur.g + 1 + penalty;
                            if (!g.count(nb) || ng < g[nb]) {
                                g[nb] = ng; par[nb] = cur.n;
                                pq.push({nb, ng, ng + h(nb, goal) * (1.0 + r * 0.2), cur.n});
                            }
                        }
                    }
                ...
                // Prepare for randomized attempts
                std::srand(std::time(nullptr) + idx);
                ...
                for (int attempt = 0; attempt < maxAttempts && !success; ++attempt) {
                    ...
                    // Different randomization and congestion factors for different attempts
                    double randomFactor = 0.1 + (0.9 * attempt / maxAttempts);
                    double congestionWeight = std::max(0.01, 0.2 - (0.19 * attempt / maxAttempts));
                    
                    // Sort sinks by difficulty for this attempt
                    std::vector<int> orderedSinks = net.sinks;
                    if (attempt % 3 == 0) {
                        ...
                        // Random order
                        std::random_shuffle(orderedSinks.begin(), orderedSinks.end());
                    }
                    
                    // Route each sink
                    for (int sink : orderedSinks) {
                        auto path = ultraSharpAStar(tree, sink, randomFactor, congestionWeight);
                    ...
                    if (!allSinksRouted) {
                        if (debug && (attempt + 1) % 5 == 0) {
                            std::cout << "Attempt " << (attempt + 1) 
                                      << ": Could not find paths to all sinks\n";
                        }
                    ...
                    // Try to reroute all affected nets with multiple strategies
                    bool allAffectedRouted = false;
                    
                    // Try multiple rerouting strategies for affected nets
                    for (int rerouteStrategy = 0; rerouteStrategy < 3 && !allAffectedRouted; ++rerouteStrategy) {
                        // Restore affected nets to empty state
                        for (size_t affectedIdx : affectedNets) {
                            results[affectedIdx].edges.clear();
                            results[affectedIdx].allSinks = false;
                        }
                        
                        // Different congestion weights for different strategies
                        double affectedCongestionWeight = 0.2;
                        if (rerouteStrategy == 1) affectedCongestionWeight = 0.1;
                        if (rerouteStrategy == 2) affectedCongestionWeight = 0.05;
                        
                        std::vector<size_t> reroutingOrder(affectedNets.begin(), affectedNets.end());
                        
                        // Sort affected nets by different strategies
                        if (rerouteStrategy == 0) {
                            // Sort by sink count (simpler nets first)
                            std::sort(reroutingOrder.begin(), reroutingOrder.end(),
                                     [&](size_t a, size_t b) {
                                         return nets[a].sinks.size() < nets[b].sinks.size();
                                     });
                        } else if (rerouteStrategy == 1) {
                            ...
                            // Random order
                            std::random_shuffle(reroutingOrder.begin(), reroutingOrder.end());
                        }
                        
                        allAffectedRouted = true;
                        for (size_t affectedIdx : reroutingOrder) {
                            // Use different routing strategies based on reroute attempt
                            RouteRet res;
                            if (rerouteStrategy == 0) {
                                // Standard routing
                                res = routeNet(nets[affectedIdx], affectedCongestionWeight);
                            } else {
                                // Custom routing with more aggressive parameters
                                const Net& affectedNet = nets[affectedIdx];
                                std::vector<std::pair<int, int>> allEdges;
                                std::unordered_set<int> affTree{affectedNet.source};
                                bool allAffSinks = true;
                                
                                for (int sink : affectedNet.sinks) {
                                    auto path = ultraSharpAStar(affTree, sink, 0.2, affectedCongestionWeight);
                        ...
                        if (allAffectedRouted) {
                            break; // Found a working rerouting strategy
                        }
                    }
                ...
                if (debug) {
                    std::cout << "Ultra-sharp routing for net " << net.id 
                              << " took " << netTime << " seconds ("
                              << (success ? "success" : "failure") << ")\n";
                }
            }
            
            // Remove fixed nets from our list for the next pass
            if (!fixedInThisPass.empty()) {
                std::sort(fixedInThisPass.begin(), fixedInThisPass.end());
                std::vector<size_t> remainingNets;
                
                std::set_difference(
                    failedNets.begin(), failedNets.end(),
                    fixedInThisPass.begin(), fixedInThisPass.end(),
                    std::back_inserter(remainingNets)
                );
                
                failedNets.swap(remainingNets);
                
                if (debug) {
                    std::cout << "Pass " << (pass + 1) << " fixed " 
                              << fixedInThisPass.size() << " nets, " 
                              << failedNets.size() << " remaining\n";
                }
            }
            
            // If we didn't fix any nets in this pass, try one more radical approach
            if (fixedInThisPass.empty() && pass == 2 && !failedNets.empty()) {
                if (debug) {
                    std::cout << "Attempting RADICAL approach for remaining " 
                              << failedNets.size() << " nets\n";
                }
                
                // Get the current number of successfully routed nets
                int currentSuccessCount = 0;
                for (const auto& result : results) {
                    if (result.allSinks && !result.removed) {
                        currentSuccessCount++;
                    }
                }
                
                // Save current state
                std::vector<uint8_t> radicalOriginalOcc = occ;
                std::vector<RouteResult> radicalOriginalResults = results;
                
                // RADICAL APPROACH: Remove up to 30% of existing nets to make space
                // for our difficult nets
                std::vector<size_t> allRoutedNets;
                for (size_t i = 0; i < results.size(); ++i) {
                    if (results[i].allSinks && !results[i].removed && 
                        std::find(failedNets.begin(), failedNets.end(), i) == failedNets.end()) {
                        allRoutedNets.push_back(i);
                    }
                }
                
                // Shuffle to randomize which nets we remove
                std::random_shuffle(allRoutedNets.begin(), allRoutedNets.end());
                
                // Take up to 30% of successfully routed nets
                size_t netsToRemove = std::min(allRoutedNets.size(), 
                                             static_cast<size_t>(allRoutedNets.size() * 0.3));
                std::vector<size_t> sacrificeNets(allRoutedNets.begin(), 
                                                allRoutedNets.begin() + netsToRemove);
                
                if (debug) {
                    std::cout << "Removing " << sacrificeNets.size() 
                              << " successfully routed nets to make space\n";
                }
                
                // Remove those nets
                for (size_t netIdx : sacrificeNets) {
                    for (auto [u, v] : results[netIdx].edges) {
                        if (occ[u]) --occ[u];
                        if (occ[v]) --occ[v];
                    }
                    results[netIdx].edges.clear();
                    results[netIdx].allSinks = false;
                    results[netIdx].removed = true;
                }
                
                // Now try to route our failed nets with ultra-low congestion weights
                std::vector<size_t> newlyRoutedNets;
                
                for (size_t idx : failedNets) {
                    const Net& net = nets[idx];
                    
                    if (debug) {
                        std::cout << "Radical attempt: Routing net " << net.id << "\n";
                    }
                    
                    // Use standard routing with near-zero congestion weight
                    auto res = routeNet(net, 0.01);
                    
                    if (res.ok) {
                        results[idx].edges = std::move(res.edges);
                        results[idx].allSinks = true;
                        results[idx].removed = false;
                        newlyRoutedNets.push_back(idx);
                ...
                // Now try to route back the nets we sacrificed
                std::vector<size_t> restoredNets;
                
                for (size_t netIdx : sacrificeNets) {
                    const Net& net = nets[netIdx];
                    
                    // Use standard routing
                    auto res = routeNet(net, 0.3);
                    
                    if (res.ok) {
                        results[netIdx].edges = std::move(res.edges);
                        results[netIdx].allSinks = true;
                        results[netIdx].removed = false;
                        restoredNets.push_back(netIdx);
                    }
                }
                
                // Count how many nets we have now
                ...
                // If we made progress, keep the new state
                if (newSuccessCount > currentSuccessCount) {
                    if (debug) {
                        std::cout << "Radical approach improved from " 
                                  << currentSuccessCount << " to " 
                                  << newSuccessCount << " routed nets\n";
                        std::cout << "Routed " << newlyRoutedNets.size() 
                                  << " new nets, restored " << restoredNets.size() 
                                  << " of " << sacrificeNets.size() << " sacrificed nets\n";
                    }
                    
                    totalSuccessfulNets += newlyRoutedNets.size();
                    
                    // Update failed nets list
                    std::vector<size_t> remainingNets;
                    
                    std::set_difference(
                        failedNets.begin(), failedNets.end(),
                        newlyRoutedNets.begin(), newlyRoutedNets.end(),
                        std::back_inserter(remainingNets)
                    );
                    ...
                    if (debug) {
                        std::cout << "Radical approach did not improve results, reverting\n";
                    }
                    occ = radicalOriginalOcc;
                    results = radicalOriginalResults;
                }
            }
        }
        ...
        if (debug) {
            std::cout << "Ultra-sharp routing completed in " << totalTime 
