...
public:
    ...
    void route() {
        ...
        // 检测拥塞节点
        std::unordered_map<int, std::unordered_set<size_t>> nodeToNets; // 记录每个节点被哪些网络使用
        ...
        for(const auto& nodeNets : nodeToNets) {
            int node = nodeNets.first;
            const auto& nets = nodeNets.second;
            if(nets.size() > 1) {
        ...
        if(!congestedNodes.empty()) {
            ...
            for(const auto& contrib : netCongestionContribution) {
                size_t idx = contrib.first;
                int contribution = contrib.second;
            ...
            for(const auto& sortedNet : sortedNets) {
                size_t idx = sortedNet.first;
                
                // 检查是否还有拥塞
                bool stillCongested = false;
                for(const auto& nodeNets : simulatedNodeToNets) {
                    const auto& nets = nodeNets.second;
                    if(nets.size() > 1) {
                        stillCongested = true;
                        break;
                ...
                if(!stillCongested) break;  // 如果没有拥塞了，停止移除
            ...
            if(debug) {
                ...
                std::cout << "Remaining successfully routed nets: " << (nets.size() - netsToRemove.size()) << "/" << nets.size() << std::endl;
            }
            
            // 输出没有被移除的网络
            for(size_t idx : outputOrder) {
                if(netsToRemove.count(idx) == 0) {
                    const auto& result = results[idx];
                ...
                if(!visited.count(node)) {
                    ...
                    if(dfs(node, -1) && !loopEdges.empty()) {
                        ...
                        if(!removed && !loopEdges.empty()) {
                            const auto& edge = loopEdges.back();
                            int u = edge.first;
                            int v = edge.second;
                            int min_node = std::min(u, v);
                            int max_node = std::max(u, v);
