...
void Router::configureRoutingParameters() {
    ...
    if (netCount <= 50) {
        ...
        maxIterations = 1;
    } else if (netCount <= 500) {
        // 小型设计：单次遍历加简单A*
        useNegotiatedRouting = false;
        maxIterations = 1;
    } else if (netCount <= 5000) {
        // 中型设计：最多2次谈判迭代
...
void Router::routeNets() {
    ...
    std::sort(results.begin(), results.end(), 
              [](const RouteResult& a, const RouteResult& b) { 
                  try {
                      return std::stoi(a.netId) < std::stoi(b.netId);
...
std::string Router::getNodeType(const std::string& nodeId) const {
...
void Router::routeIteration(int iteration, float congestionMultiplier) {
    ...
    std::unordered_set<std::string> fixedNets;
    if (iteration > 0) {
    ...
    for (size_t i = 0; i < nets.size(); ++i) {
        ...
        if (result.success) {
            ...
            // 更新缓存 - 收集源和目标类型
            std::string sourceType = getNodeType(net.source);
            std::vector<std::string> sinkTypes;
            for (const auto& sink : net.sinks) {
                sinkTypes.push_back(getNodeType(sink));
            }
            
            // 添加到网络模式缓存
            std::string patternKey = routeCache.generateNetPatternKey(sourceType, sinkTypes);
            routeCache.addNetPattern(patternKey, result.connections);
            
            // 记录节点类型使用情况
            routeCache.recordNodeTypeUsage(sourceType);
            for (const auto& sinkType : sinkTypes) {
                routeCache.recordNodeTypeUsage(sinkType);
            }
        }
...
RouteResult Router::tryRouteCached(const Net& net, const std::unordered_set<std::string>& avoidNodes) {
    RouteResult result;
    result.netId = net.id;
    ...
    if (routeCache.hasNetPattern(patternKey)) {
        // 尝试使用缓存的连接模式
        std::vector<std::pair<std::string, std::string>> cachedConnections = 
            routeCache.getNetConnections(patternKey);
        
        if (!cachedConnections.empty()) {
            // 验证这些连接是否可用（不与避开的节点冲突）
            bool conflictFound = false;
            std::unordered_set<std::string> usedNodes;
    ...
    std::unordered_set<std::string> treeNodes = {net.source};
    std::vector<std::pair<std::string, std::string>> treeConnections;
    ...
    for (const auto& sink : net.sinks) {
        ...
        for (const auto& treeNode : treeNodes) {
            ...
            if (routeCache.hasPath(treeNode, sink)) {
                std::vector<std::string> path = routeCache.getPath(treeNode, sink);
...
RouteResult Router::routeNet(const Net& net, const std::unordered_set<std::string>& fixedNets, float congestionMultiplier) {
    RouteResult result;
    result.netId = net.id;
    result.netName = net.name;
    ...
    // 构建要避开的节点集合
    std::unordered_set<std::string> avoidNodes = usedResources;
...
std::vector<std::string> Router::optimizedAStarSearch(
    ...
    float targetX = 0.0f, targetY = 0.0f;
    {
        ...
        if (targetIt != nodes.end()) {
            ...
            targetX = (targetNode.begin_x + targetNode.end_x) / 2.0f;
            targetY = (targetNode.begin_y + targetNode.end_y) / 2.0f;
        }
    }
    ...
    float hStart = 0.0f;
    {
        auto startIt = nodes.find(start);
        if (startIt != nodes.end()) {
            const auto& startNode = startIt->second;
            float x = (startNode.begin_x + startNode.end_x) / 2.0f;
            float y = (startNode.begin_y + startNode.end_y) / 2.0f;
    ...
    while (!openList.empty() && iterations < MAX_ITERATIONS) {
        ...
        // 获取当前节点
        std::string current = openList[current_idx].nodeId;
        ...
        for (const auto& neighbor : adjIt->second) {
            ...
            if (!inOpenList || tentative_g < gScore[neighbor]) {
                ...
                if (neighborIt != nodes.end()) {
                    const auto& neighborNode = neighborIt->second;
                    float x = (neighborNode.begin_x + neighborNode.end_x) / 2.0f;
                    float y = (neighborNode.begin_y + neighborNode.end_y) / 2.0f;
...
Router::buildSteinerTree(
    ...
    std::unordered_set<std::string> treeNodes = {source};
    std::vector<std::pair<std::string, std::string>> treeConnections;
    std::unordered_set<std::string> remainingSinks(sinks.begin(), sinks.end());
...
void Router::updateCongestionMap(const std::unordered_set<std::string>& nodes) {
...
Router::Point Router::getNodeCenter(const std::string& nodeId) {
    auto it = nodes.find(nodeId);
    if (it != nodes.end()) {
        const auto& node = it->second;
        return {(node.begin_x + node.end_x) / 2.0f, (node.begin_y + node.end_y) / 2.0f};
    }
    return {0.0f, 0.0f};
}

float Router::manhattanDistance(const std::string& node1, const std::string& node2) {
...
void Router::logProgress(size_t netsRouted, size_t totalNets, const std::string& netId) {
    if (!debug) return;
