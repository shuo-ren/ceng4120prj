...
DataLoader::DataLoader(int n) : threads(n < 1 ? 1 : n) {}

// ---------- device ----------
void DataLoader::loadDevice(const std::string& file){
...
// ---------- netlist ----------
void DataLoader::loadNetlist(const std::string& file){
    auto t0=std::chrono::high_resolution_clock::now();
    std::ifstream fin(file);
    if(!fin) throw std::runtime_error("open netlist "+file);

    nets.clear();
    std::string line; std::getline(fin,line);         // header or first
    bool hasHeader=true;
